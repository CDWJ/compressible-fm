...
@ti.kernel
def calculate_gradient(h:ti.template(),
    h_gradient:ti.template(),
    dx:float
):
    for i,j in h:
        vec = ti.Vector([
            (sample(h,i+1,j)-sample(h,i-1,j))/2/dx,
            (sample(h,i,j+1)-sample(h,i,j-1))/2/dx,
        ])
        h_gradient[i,j] = vec.norm()

@ti.kernel
def scalar2vector(sf:ti.template(),vf:ti.template(),ind:int):
    for I in ti.grouped(sf):
        vf[I][ind]=sf[I]

@ti.kernel
def vector2scalar(vf:ti.template(),sf:ti.template(),ind:int):
    for I in ti.grouped(sf):
        sf[I]=vf[I][ind]
...
@ti.func
def interp_MAC_divergence_u(u_x, u_y, p, dx):
    ...
    return grad_u_x_p[0]+grad_u_y_p[1]
...
@ti.func
def interp_u_MAC_grad_1(u_x, u_y, p, dx):
    u_x_p, grad_u_x_p = interp_grad_1(u_x, p, dx, BL_x=0.0, BL_y=0.5)
    u_y_p, grad_u_y_p = interp_grad_1(u_y, p, dx, BL_x=0.5, BL_y=0.0)
