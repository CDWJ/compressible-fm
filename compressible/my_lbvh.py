...
class BVHNode:
    ...
    triangle_index: ti.i32  # Index of the triangle (for leaf nodesc)
...
class LBVH:
    def __init__(self, vertices, faces, vn, fn):
        ...
        self.vertices.copy_from(vertices) # Taichi field of shape (num_vertices, 3)
        self.faces.copy_from(faces)
    ...
    def update_bvh_tree(self, new_vertices):
        self.new_vertices.copy_from(new_vertices)

        self.changed_triangles.fill(0)
        self.mark_changed_triangles()
        self.vertices.copy_from(new_vertices)
